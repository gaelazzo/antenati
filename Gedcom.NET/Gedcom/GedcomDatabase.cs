...
namespace Gedcom
{
	...
	public class GedcomDatabase : IEnumerator
	{
		...
		public GedcomDatabase()
		{
            ...
            _surnames = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
		}
        ...
        protected virtual void BuildSurnameList(GedcomIndividualRecord indi)
        {
            foreach (GedcomName name in indi.Names)
            {
				...
				if (!string.IsNullOrEmpty(name.SurnamePrefix)) {
					surname = name.SurnamePrefix + " " + surname;
				}
								
                    
                
				//if (name.Prefix != null) { surname = name.Prefix + " " + surname; }
				//if (name.Suffix != null) { surname += surname +" "+ name.Suffix; }
